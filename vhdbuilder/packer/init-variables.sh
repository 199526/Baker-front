...
if [ "$OS_SKU" == "Windows" ]; then
	imported_windows_image_name=""
	source $CDIR/windows-image.env

	# Set the base image sku and version from windows-image.env
	case "${WINDOWS_SKU}" in
	...
	*)
	...
	# Set the base image url if the pipeline variable is set and mode is not sigMode
